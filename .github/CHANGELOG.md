...
- `PyNsga2Optimizer::score()` and `PyMopsoOptimizer::score()` silently returned a fabricated `0.0` when `fit()` was called with an objective returning fewer than 2 values (hypervolume is undefined for <2 objectives), instead of surfacing that misuse. Both now raise `ValueError` instead, matching this project's established "raise rather than fabricate a number" pattern. Found via a second pass over `docs/QA.md`.
