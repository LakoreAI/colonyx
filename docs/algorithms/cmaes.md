...
<figure markdown>
![Four-step CMA-ES loop: sample the generation from the current mean and covariance, rank it and recombine the best mu into a new mean, adapt the global step size sigma from the evolution path, then adapt each axis's variance from the best mu's spread and repeat](../assets/diagrams/cmaes.svg)
<figcaption>Only the best half of each generation ever influences the next mean, variance, or step size — the discarded half existed purely to be ranked against.</figcaption>
</figure>
